...
struct LassoControls: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Text(session.tool == .marquee ? "Marquee" : session.tool == .wand ? "Magic Wand" : session.tool == .objectSelection ? "Object Selection" : "Lasso").font(ToolHeaderStyle.titleFont)
            ...
            if session.tool == .lasso || session.tool == .wand || session.tool == .objectSelection || (session.tool == .marquee && session.marqueeKind == .ellipse) {
